...
class CNN1dlayer(nn.Module): #Normal CNN layer with dropout and GELU activation function 
...
class CNN1dlayerNoAct(nn.Module): #Nromal CNN layer with drouput and without activation function
    def __init__(self, in_channels, out_channels, kernel_size, stride, dropout_p=0.1, bias=False, padding=0):
        super().__init__()
        self.in_conv_dim = in_channels
        self.out_conv_dim = out_channels
...
class ResnetConfig:
    def __init__(
        ...
        print('ResNet Initiazlied to ', self.output_dim)
...
class Resnet1dBlock(nn.Module): #CNN W/ Activation -> CNN W/O Activation -> 
    def __init__(self, in_channels, out_channels, kernel_size, stride, dropout_p=0.05, downsample=False, **kwargs):
        super().__init__()
...
class ResnetClassifier(nn.Module):
    def __init__(self, config=None, num_classes=None):
        ...
        self.avgpool = nn.AdaptiveMaxPool1d(1) #takes last layer, and outputs a [(batchsize=1), channel of last resnet layer, 1] array
